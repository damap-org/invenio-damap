...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initDamapPopup() {
    ...
    let $popup = $("<div></div>", { id: "popup", class: "ui modal" }).appendTo(
      "body"
    );
    // popup header
  ...
  function fillWithDmps(response) {
    ...
    $.each(response["hits"]["hits"], function (k, v) {
      ...
      if (
        v.datasets?.some(
          (dataset) => dataset.datasetId?.identifier === window.location.href
        )
      ) {
        let $info_div =
          "<div class='ui label'><i class='info icon'></i>Linked</div>";
        $buttonDiv.append($info_div);
      }

      // define the "add" button and append it to the wrapper
      let $add_button = $(
        '<button class="ui button" type="button"><i aria-hidden="true" class="plus icon"></i>Add</button>'
      );
      ...
      $add_button.on("click", async function (e) {
        ...
        try {
          await http.post(
            `/api/invenio_damap/damap/dmp/${v.id}/dataset/${$recid}`,
            body
          );
      ...
      $("<div class='header'>" + v.project.title + "</div>").appendTo(
        "#dmp-" + v.id
      );
      $(
        "<div class='description'>" + v.project.description + "</div>"
      ).appendTo("#dmp-" + v.id);
    });
  }
});
